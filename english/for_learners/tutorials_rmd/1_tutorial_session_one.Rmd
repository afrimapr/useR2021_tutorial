...
1. `install.packages("[package_name]")`               is needed only once to install a package
from the internet (replace [package_name] with the name of your package)
...
To check that the packages have been installed, try running the `library([package_name])` commands below. If they have been installed, nothing should happen. R will only give a message if thee is a problem but not when this command is run successfully. 
