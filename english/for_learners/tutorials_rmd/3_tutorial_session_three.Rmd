...
3. data referenced by place names (a bit trickier, we didn't cover in detail today but provide pointers in section E below)
...
Google searches may lead you to stackoverflow, a super-useful Q&A for code developers. When you get more familiar with it you can also start searches from there, and even register to ask and answer questions yourself.

In both of these approaches your challenge will be to filter a useful answer that helps you solve your problem, from probably lots of other results that are either not quite relevant, or are outdated, or you don't understand. All of these happen to us.
...
1. coordinate data brought by you   (section ...) [Andy can we organise the section lettering so we can point from here]
2. coordinate data found online     (section ...)
3. data referenced by place names   (section ...)
4. data we have provided            (section ...)
...
A nice website offering tutorials and pointers to online spatial data that can be mapped in R. Some of these tutorial are a little more advanced than those we have introduced here.


## C. Uploading data to RStudio Cloud

To map data on RStudio Cloud that you've brought or found, you will first need to upload by clicking on `Upload` in the Files pane (typically the bottom right pane). We suggest you save to a `data` folder in the directory where you are running the R script (for this tutorial it is `english/for_learners/data`). 
