...
4. Data referenced by place names (a bit trickier, we didn't cover in detail today but provide pointers in section H below)
...
1. Coordinate data brought by you   (Section D)
2. Coordinate data found online     (Section E)
3. Data we have provided            (Section F)
4. Data referenced by place names   (Section H)
...
To map data on RStudio Cloud that you've brought or found, you will first need to upload by clicking on `Upload` in the Files pane (typically the bottom right pane). We suggest you save to a `data` folder in the directory where you are running the R script (for this tutorial it is `for_learners/data`). 
...
If you have time in this tutorial, try your hand at these extra steps. Or try them after the tutorial. Please do reach out after the course to the trainers to address any difficulties you come across.
