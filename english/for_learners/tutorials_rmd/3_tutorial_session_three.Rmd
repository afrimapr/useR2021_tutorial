...
By the end of the session, we would like you to email a screenshot of your mapping efforts - it doesn't matter how far you get. We will compile these into an document to send out to all participants afterwards.
...
Here is a list of [super helpful resources and links](https://support.rstudio.com/hc/en-us/articles/200552336-Getting-Help-with-R) where you can look for answers to your questions or find ideas for your analyses.
...
![**Fig1. Mapping steps**](../../for_instructors/images/steps.jpg)
...
### Add layers from the objects in `afrilearndata`to your map

* Use the `sf` object you've created from your own data as well as objects like `africapitals` from the `afrilearnrdata` and map using `tmap` (hint: use `tm_shape` & `tm_symbols`)
...
## **Please remember that the interactive maps will not show up in a static PDF document if you run the `knit` command.**
