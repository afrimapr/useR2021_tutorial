...
# Upload your data to RStudio Cloud: click on `Upload` in the Files pane (typically the bottom right pane). We suggest you save to a `data` folder in the directory where you are running the R script (for this tutorial it is `for_learners/data`)
...
dfhealth <- read_csv("/cloud/project/english/for_learners/data/[your_data].csv") # replace `[your_data].csv` with the filename of your data

# if running the project locally, replace the directory path to your file (i.e. '/cloud/project/english/for_learners/data/' above) with the path to where the file is located on your computer.
...
# Download the online data to your computer. Then, upload this data to RStudio Cloud: click on `Upload` in the Files pane (typically the bottom right pane). We suggest you save to a `data` folder in the directory where you are running the R script (for this tutorial it is `for_learners/data`)
...
# 1. Practice exploring what the object contains (hint: e.g. `head`, `str`, `names`, `class`). Add / remove the # in front of the lines one by one
head(sfhealth)
#str(sfhealth)
#names(sfhealth)
#class(sfhealth)
